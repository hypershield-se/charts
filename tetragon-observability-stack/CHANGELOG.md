...
### Added
- values.schema.json for automatic values validation
- JSON Schema validation for required fields and types
- Conditional validation for mandateUrl when mandateSource is url
- Format validation for otelGatewayEndpoint (hostname:port pattern)
- Go duration format validation for mandateRefreshPeriod
- Enum validation for mandateSource and policy modes

### Improved
- Better error messages when required values are missing
- IDE support for auto-completion and inline documentation
